...
class Behavior:
    ...
    def parse(self, alg, cur, data):
        ...
        while i<len(alg):
            if alg[i]<4:
                i += ((data[alg[i]][1] =='self')or(data[alg[i]][1] =='barrier'))*(data[alg[i]][2] <= 10) + 1 + (data[alg[i]][1] == 'food')*2
            elif alg[i]<8:
    ...
    def run(self, kwargs: KeyWordArguments):
        ...
        if self.prevdeaths and deaths>self.prevdeaths:
            print('*', end='')
            self.population[self.pop_index][1] = self.prevcycles
            self.pop_index += 1
        if self.pop_index >= self.pop_len:
            ...
            self.population = succ_pop+[self.mutate(succ_pop[x%(save_rate)], 
                                                             self.actions, 
                                                             -x%save_rate+2, 3) 
    for x in range(self.pop_len-save_rate)]
